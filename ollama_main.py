...
# thread-thrashing. Everything else (LanguageTool, the local LLM, saving to
# the DB) still runs freely in parallel across users; only the actual GPU-bound
...
print("Configuring Ollama client...")
# Ollama has no official Python SDK requirement - it's a plain local HTTP
...
def _call_ollama(prompt: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"running, and is the model pulled ('ollama pull {OLLAMA_MODEL}')? "
            f"Underlying error: {e}"
        )
    return body.get("response", "")


def _run_ollama_review(transcript: str, wpm: float, filler_count: int, long_pauses: int):
    """Returns (corrected_transcript, extra_mistakes, score, score_reason,
    criteria, strength, improve).

    Same shape and rubric math as the Claude version (see main.py) - this
    file exists purely to compare a local model's output quality/speed
    against Claude on real recordings, with everything else (Whisper,
    LanguageTool, DB, endpoints) identical between the two.
    ...
    to a /10 total, per the school's rubric.

    Local review is treated as REQUIRED, not optional, same as the Claude
    ...
    for attempt in range(1, OLLAMA_MAX_RETRIES + 1):
        try:
            ...
            if attempt < OLLAMA_MAX_RETRIES:
                wait = OLLAMA_RETRY_BASE_SEC * (2 ** (attempt - 1))
                print(f"[ollama] retrying in {wait:.0f}s...")
                time.sleep(wait)
    ...
    raise RuntimeError(
        f"Local (Ollama) review failed after {OLLAMA_MAX_RETRIES} attempts. "
...
def analyze_audio(audio_path: str) -> dict:
    ...
    ollama_future = _review_executor.submit(
        _run_ollama_review, transcript, wpm, len(fillers), long_
    )
    grammar = lt_future.result()
    # Local (Ollama) review is required now (see _run_ollama_review's retry
    # logic) — no timeout-and-skip here. If it ultimately fails after all
    # retries, that exception propagates up and the /api/analyze request
    # fails with a clear error instead of silently shipping a session
    # without it.
    corrected, ollama_mistakes, score, score_reason, criteria, strength, improve = ollama_future.result()
    grammar.extend(ollama_mistakes)
    t_review = time.time()
    print(f"[timing] grammar review (LanguageTool + local Ollama, parallel): {t_review - t_whisper:.1f}s")
...
async def analyze(
    ...
    try:
        ...
        # Most likely cause here is _run_ollama_review exhausting its
        # retries (see OLLAMA_MAX_RETRIES) — surfaced as a clean error
        # instead of a raw 500 traceback, since local review is required.
...
def get_rubric():
    ...
    what the local model is actually scoring against."""
    return {
        "criteria": [
            {
                "key": key,
