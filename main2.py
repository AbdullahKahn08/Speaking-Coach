...
# thread-thrashing. Everything else (LanguageTool, Claude, saving to the DB)
...
CLAUDE_TIMEOUT_SEC = float(os.environ.get("CLAUDE_TIMEOUT_SEC", "25"))
# Claude review is required (see _run_claude_review) — these control how hard
# it retries before finally giving up and failing the request.
CLAUDE_MAX_RETRIES = int(os.environ.get("CLAUDE_MAX_RETRIES", "5"))
CLAUDE_RETRY_BASE_SEC = float(os.environ.get("CLAUDE_RETRY_BASE_SEC", "2"))
...
# Single source of truth for the rubric text — used to BOTH build the Claude
...
# teacher sees always matches exactly what Claude was actually told to judge
...
def _run_claude_review(transcript: str, wpm: float, filler_count: int, long_pauses: int):
    ...
    Claude review is treated as REQUIRED, not optional: LanguageTool alone
    misses meaning-level mistakes, which is the whole reason this pass was
    ...
    CLAUDE_MAX_RETRIES attempts.
    """
    if not transcript:
        return transcript, [], None, None, {}, "", ""
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise RuntimeError(
            "ANTHROPIC_API_KEY is not set. Claude review is required for "
            "this app to work — set the env var and restart the server."
        )
    ...
    for attempt in range(1, CLAUDE_MAX_RETRIES + 1):
        try:
            resp = _claude.messages.create(
                model=CLAUDE_MODEL,
                # 1500 was too tight: the rubric response includes 4 criteria
                # comments, a full corrected transcript, and a potentially
                # long mistakes list — on longer transcripts this got cut
                # off mid-string, producing an "Unterminated string" JSON
                # parse error on every retry (a hard truncation, not a flaky
                # failure, so retrying alone never fixed it).
                max_tokens=4096,
            ...
            if resp.stop_reason == "max_tokens":
                # Claude filled the entire token budget without finishing —
                # the JSON is guaranteed incomplete. Raise a clear message
                # immediately instead of letting json.loads fail with a
                # confusing "unterminated string" error further down.
                raise RuntimeError(
                    "Claude's response was cut off by hitting max_tokens "
                    "before finishing the JSON — the transcript may be "
                    "unusually long. Consider raising max_tokens further."
                )
            ...
            last_err = e
            print(f"[claude] attempt {attempt}/{CLAUDE_MAX_RETRIES} failed: {e}")
            if attempt < CLAUDE_MAX_RETRIES:
                wait = CLAUDE_RETRY_BASE_SEC * (2 ** (attempt - 1))
                print(f"[claude] retrying in {wait:.0f}s...")
                time.sleep(wait)
    ...
    raise RuntimeError(
        f"Claude review failed after {CLAUDE_MAX_RETRIES} attempts. "
        f"Last error: {last_err}"
    )


# LanguageTool and Claude both only need the transcript, so run them
# concurrently instead of back-to-back — this is usually the single biggest
# easy win since Claude is a network call that can take several seconds.
...
def analyze_audio(audio_path: str) -> dict:
    ...
    claude_future = _review_executor.submit(
        _run_claude_review, transcript, wpm, len(fillers), long_
    )
    grammar = lt_future.result()
    # Claude review is required now (see _run_claude_review's retry logic) —
    ...
    corrected, claude_mistakes, score, score_reason, criteria, strength, improve = claude_future.result()
    grammar.extend(claude_mistakes)
    t_review = time.time()
    print(f"[timing] grammar review (LanguageTool + Claude, parallel): {t_review - t_whisper:.1f}s")
...
async def analyze(
    ...
    try:
        ...
        # Most likely cause here is _run_claude_review exhausting its
        # retries (see CLAUDE_MAX_RETRIES) — surfaced as a clean error
        # instead of a raw 500 traceback, since Claude review is required.
...
def get_rubric():
    ...
    what Claude is actually scoring against."""
    return {
        "criteria": [
            {
                "key": key,
